...
function Navbar() {
    ...
    return (
        <>
            <div className='h-16 md:h-20 w-full px-4 md:px-10 py-4 md:py-10 font-["Neue Montreal"] flex justify-between items-center relative z-50'>
                ...
                <div className='hidden lg:flex links gap-6 xl:gap-10'>
                    {navItems.map((item, index) => (
                        <a key={index} 
                           href="#"
                           className={`text-base xl:text-lg font-light hover:underline transition-all duration-200 ${index === 4 && "xl:ml-32"}`}
                        >
                            {item}
                        </a>
                    ))}
